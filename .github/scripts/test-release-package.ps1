...
$fixture = @'
<div align="center">
# ![PlayerNotes](https://readme-typing-svg.demolab.com/?lines=PlayerNotes)
...
Preserve `/pn <name>`, `<details>`, and [ordinary links](https://example.com/a_(b)).
</details>
```html
...
foreach ($want in @('# PlayerNotes', '### Feature', 'Keep the summary.', '[Download](https://example.com/releases)',
    '`/pn <name>`', '`<details>`', '[ordinary links](https://example.com/a_(b))', '<details>Preserve this code.</details>')) {
...
$scratch = Join-Path ([IO.Path]::GetTempPath()) ('playernotes-release-test-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    foreach ($required in @('playernotes.lua', 'README.md', 'LICENSE', 'sounds/friend_alert.wav', 'sounds/avoid_alert.wav')) {
